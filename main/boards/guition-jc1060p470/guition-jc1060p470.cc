 ...
 * =============================================================================
 *  Hardware:
 *    - ESP32-P4 (engineering sample v1.0 / v1.3) + ESP32-C6 (Wi-Fi 6)
 *    - Display: 7" 1024x600 IPS, JD9165, MIPI-DSI 2-lane
 *    - Touch: GT911 (I2C, shared with ES8311)
 *    - Audio: ES8311 + PA
 *    - Camera: MIPI-CSI (EspVideo)
 *
 *  Based on official Guition P4-series demo and xiaozhi-esp32 board API
 *  (see waveshare/esp32-p4-wifi6-touch-lcd).
 *
 *  IDF: 5.5.x recommended for this board bring-up
 *  Dependency: idf.py add-dependency "espressif/esp_lcd_jd9165^1.0.3"
 * =============================================================================
 */
...
private:
    ...
    void EnableDsiPhyPower() {
        ...
        ESP_LOGI(TAG, "MIPI DSI PHY LDO ch%d @ %d mV", MIPI_DSI_PHY_PWR_LDO_CHAN,
...
// Manual init: IDF 6.0.x requires designated-initializer field order
        ...
        ESP_LOGI(TAG, "JD9165 1024x600 display initialized");
    }
    ...
    void InitializeTouch() {
        ...
        if (lv_display == nullptr) {
            ESP_LOGE(TAG, "No LVGL display — touch not registered");
            return;
        }
        ...
        if (touch_indev_ == nullptr) {
            ESP_LOGE(TAG, "Failed to register GT911");
        } else {
    ...
    void InitializeSdCard() {
        ESP_LOGI(TAG, "SD init (may conflict with C6 Wi-Fi)");
        ...
        if (ret != ESP_OK) {
            ESP_LOGW(TAG, "SD power not enabled: %s", esp_err_to_name(ret));
            return;
        }
        ...
        if (ret != ESP_OK) {
            ESP_LOGW(TAG, "SD not mounted (OK if Wi-Fi is primary): %s",
                     esp_err_to_name(ret));
    ...
    void InitializeCamera() {
        ESP_LOGI(TAG, "MIPI-CSI camera init");
    ...
    void InitializeFonts() {
        ...
        if (current_theme != nullptr) {
            ...
            if (text_font != nullptr && text_font->font() != nullptr) {
                ESP_LOGI(TAG, "Font loaded: line_height=%d",
...
public:
    GuitionJC1060P470Board() : boot_button_(BOOT_BUTTON_GPIO) {
        ...
        ESP_LOGI(TAG, "Guition JC1060P470C board ready");
    }
